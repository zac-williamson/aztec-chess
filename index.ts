...
async function main() {
    ...
    const bobToken = await BobTokenContract
     ...
     let whiteState = await bobToken.methods.__empty_white_state().simulate({from: giggleAddress});
      console.log("white state ", whiteState);

    
    console.log("about to call method that errors?");
     await bobToken.methods
        .args_hash_err(whiteState).send({ from: giggleAddress}).wait();

    console.log("creating game state");
    let gameState = await bobToken.methods.__empty_game_state().simulate({from: giggleAddress});
 

    let blackState = await bobToken.methods.__empty_black_state().simulate({from: giggleAddress});
        console.log("black state");

    // NOTE: in a real game these secrets should be random and not shared.
    whiteState.state.encrypt_secret = 1;
    whiteState.state.mask_secret = 2;
    blackState.state.encrypt_secret = 3;
    blackState.state.mask_secret = 4;
    let whiteEncryptSecret = 1;
    let whiteMaskSecret = 2;
    let blackEncryptSecret = 3;
    let blackMaskSecret = 4;
    ...
    gameState = await bobToken.methods.__commit_to_user_secrets(
      gameState,
      whiteEncryptSecret,
      whiteMaskSecret,
      0 // 0 = white
    ...
    console.log("game state? ", (gameState));
        ...
        console.log("secret hashes? ", (gameState.state.mpc_state.user_encrypt_secret_hashes));

     //   console.log("white state secrets ", whiteState.)
    let move = await bobToken.methods.__create_move(0, 1, 0, 3).simulate({from: giggleAddress});

    let new_user_state = await bobToken.methods.update_user_state_from_move(true, whiteState, move, 0).simulate({ from: giggleAddress});
